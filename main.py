...
def vaild_point(EulerAngles,LBH_dd,LBH_plane,uv):
    ...
    EulerAngles = [i*DEG2RAD for i in EulerAngles]
    ...
    Rxyz_last = llh2ecef(LBH_dd)
    enu_last = ecef2enu(LBH_dd, Rxyz_last)
    print("探测器东北天坐标",enu_last)

    Rxyz_now = llh2ecef(LBH_plane)
    enu_now = ecef2enu(LBH_dd, Rxyz_now)
    print("目标东北天坐标", enu_now)
    print("目标与探测器水平距离", math.sqrt(enu_now[0] * enu_now[0] + enu_now[1] * enu_now[1]))



    plane_vec = np.array(enu_now)
    XYZ_vector = np.matmul(R_yanzheng,plane_vec)
    # XYZ_vector1 = np.matmul(R_yanzheng_wai,plane_vec)
    print("目标相对与探测器坐标",XYZ_vector)
    # print("验证坐标系旋转",XYZ_vector1)
    XYZ2XZY = np.matmul(np.array([[1, 0, 0], [0, 0, -1], [0, 1, 0]]), XYZ_vector)
    print("验证XYZ坐标",XYZ2XZY)
    XYZ2XZY = XYZ2XZY / XYZ2XZY[2]
    print("验证XYZ坐标",XYZ2XZY)
    print("uv",uv)
    uv_vector_fromK = np.matmul(K,XYZ2XZY)


    uv_vector_fromratio = computrdUVPos(XYZ_vector, 12.5, 10)
...
if __name__ == '__main__':
    ...
    K = np.array([[8020, 0.0, 320.0], [0.0, 8020, 256], [0.0, 0.0, 1.0e+00]])
    print("K：",K)

    #前后帧目标经纬高
    target_blh = [[84.483139,37.329095,10000],[84.483139,37.329095,10000]]
    #前后帧探测器经纬高
    dd_blh = [[84.517228,37.347878,10708],[84.5068,37.342108,10539]]
    #前后帧探测器-航向-俯仰-滚转
    dd_y_p_r = [[123.3265,-7.347000,3.855500],[122.26,-6.98,5.27]]
    #前后帧成像坐标，u：x轴；v:y轴
    uv = [[266,392],[229,468]]

    #前后帧图片路径
    image1_path = 'D:\Work_space\dk\Epipolar_Geometry\img1.jpg'
    image2_path = 'D:\Work_space\dk\Epipolar_Geometry\img2.jpg'
    ...
    ypl_last = [yaw_last_frame * DEG2RAD, pitch_last_frame * DEG2RAD, roll_last_frame * DEG2RAD]
    print("前一帧机体坐标系(航向，俯仰，滚转)：", yaw_last_frame, pitch_last_frame, roll_last_frame)
    ...
    print("当前帧机体坐标系(航向，俯仰，滚转)：", yaw_now_frame, pitch_now_frame, roll_now_frame)
    l_last_frame = dd_blh[0][0];
    b_last_frame = dd_blh[0][1];
    h_last_frame = dd_blh[0][2];
    LBH_last = [l_last_frame, b_last_frame, h_last_frame]
    print("前一帧机体坐标(经度，维度，高度)：", l_last_frame, b_last_frame, h_last_frame)
    l_now_frame = dd_blh[1][0];
    b_now_frame = dd_blh[1][1];
    h_now_frame = dd_blh[1][2];
    LBH_now = [l_now_frame, b_now_frame, h_now_frame]
    print("当前帧机体坐标(经度，维度，高度)：", l_now_frame, b_now_frame, h_now_frame)
    EulerAngles = [(yaw_now_frame - yaw_last_frame) * DEG2RAD, (pitch_now_frame - pitch_last_frame) * DEG2RAD, 0.0]
    print("欧拉角弧度", EulerAngles[0], EulerAngles[1], EulerAngles[2])
    # Zyaw = np.array([[math.cos(EulerAngles[0]),math.sin(EulerAngles[0]),0],[-math.sin(EulerAngles[0]),math.cos(EulerAngles[0]),0],[0,0,1]]);
    # Xpitch = np.array([[1,0,0],[0,math.cos(EulerAngles[1]),math.sin(EulerAngles[1])],[0,-math.sin(EulerAngles[1]),math.cos(EulerAngles[1])]]);
    # Yroll = np.array([[math.cos(EulerAngles[2]),0,-math.sin(EulerAngles[2])],[0,1,0],[math.sin(EulerAngles[2]),0,math.cos(EulerAngles[2])]]);
    ...
    print("上一帧探测器东北天坐标", enu_last)
    Rxyz_now = llh2ecef(LBH_now)
    t = enu_now = ecef2enu(LBH_last, Rxyz_now)
    print("当前帧探测器东北天坐标", enu_now)

    #开始求R,T
    #x2 = Rdd2图 * R欧2 * [（Rdd2图 * R欧1）逆 * x1  +  北天冬delta_t  ]
    oula_xuanzhuan1 = oula_xuanzhuan(ypl_last)
    oula_xuanzhuan1_inv = np.linalg.inv(oula_xuanzhuan1)
    oula_xuanzhuan2 = oula_xuanzhuan(ypl_now)
    R_dd2image = np.array([[1, 0, 0], [0, 0, -1], [0, 1, 0]])
    R_dd2image_inv = np.linalg.inv(np.array([[1, 0, 0], [0, 0, -1], [0, 1, 0]]))
    print("欧拉旋转矩阵1",oula_xuanzhuan1)
    print("欧拉旋转矩阵2",oula_xuanzhuan2)
